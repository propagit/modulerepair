...
class Order_model extends CI_Model {
	...
	function get_customer($name='',$id='')
	{
		...
		if($name!='')
		{
			$sql = "Select customers.*
					from customers
					where (CONCAT(customers.firstname, ' ', customers.lastname) LIKE '%$name%' OR customers.lastname LIKE '%$name%' OR customers.firstname LIKE '%$name%')";
	...
	function search_v2($customer_name,$order_id,$from_date,$to_date,$by_keyword,$by_status, $by_typecustomer,$by_payment,$by_ord_status,$cust_id) {
		...
		if($customer_name || $order_id || $from_date || $to_date || $by_keyword || $by_payment || $cust_id || $by_ord_status)
		{
			...
			$sql = "Select orders.customer_id, orders.id, orders.total, orders.order_status, orders.status, orders.order_time
