...
class System_model extends CI_Model {
	...
	function to_short($txt,$long)
	{
		return substr($txt, 0,$long);
	}
	...
	function best_customer() {
		...
		if ($result) {
			...
			$row = $query->first_row('array');
			if ($row) {
				return substr($row['firstname'].' '.$row['lastname'],0,33).' ($'.number_format($result['total'],2,'.',',').')';
			}
		}
		return 'N/A';
	}
	...
	function best_category() {
		$sql = "SELECT `products_categories`.`category_id`, count(`products_categories`.`category_id`) as `total`
				...
				WHERE `orders`.`id` = `carts`.`order_id` 
	...
	function best_product() {
		$sql = "SELECT `carts`.`product_id`,sum(`carts`.`quantity`) as `total` 
				FROM `orders`,`carts` 
				WHERE `orders`.`id` = `carts`.`order_id` and `orders`.`status` = 'successful'
				GROUP BY `carts`.`product_id` 
				ORDER BY `total` DESC";
		$query = $this->db->query($sql);
		$result = $query->first_row('array');
		if ($result) {
			...
			$query = $this->db->get('products');
			$row = $query->first_row('array');
			return substr($row['title'],0,33).' ('.$result['total'].')';
		}
		return 'N/A';
	}
